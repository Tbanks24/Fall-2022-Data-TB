...
```{r echo=FALSE}
census_api_key("8db63c57e4c527584e58c567b7d791738146383f", install=TRUE)
```
...
**Answer**No, the margin of error is not signififcant. 
...
**Answer**
No one saw their incoime fall between thos two years. 
```{r}
md10 %>%
...
**Answer** Why don't they produce an estimate value? 

```{r}
md_zips_with_acs <- md_zips_with_acs %>%
  mutate(min=(estimate-moe), max=(estimate+moe)) %>% 
arrange(md_zips_with_acs, desc(estimate))
