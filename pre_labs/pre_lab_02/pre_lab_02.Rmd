...
**Answer** This created a new column titled percent_election day with new values for the percentage of votes on election day in decimal form.  
...
**Answer**
The results are now put in descending order based on the election day percent. 
...
**Answer**I think it's probably more useful.For descending order, the person who had only 8 election day votes as their total number of votes and it shows that the percentage of their votes the person recieved on election day was 100%. This number doesn't really mean anything since the person only received 8 votes and they were all on election day. 
...
Str_to_lower- Converts a string to lowercase (all letters in word are lowercase.)
...
**Answer** 
It's looking at the amount category. Based on that amount, the column amount category shows what range the value falls into. Which of the given equations applies to it. The mutate function added the amount_category value. 
...
**Answer** Because we want to keep the data that only shows results from the Prince George's jurisdiction and the one that only shows the district, office, name, party, and votes separate. You may need to use these two groups separately later on so you should store them in two different variables.  
...
**Answer:** The format of the date changes to year-month-day format. 
...
**Answer** The dates now go back to the year 2020 instead of just all being in  2022. The values still go by year-month-date format. All these dates also have the same day (the first for each month) despite the year differences. 
