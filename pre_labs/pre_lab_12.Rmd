...
**Answer**A chart with 19 rows and 22 columns. The data shows how people have registered to vote, how many removals each party had depending on the reason for them. The word removals seemed misplaced and should be moved over to where the data is located. 
...
**Answer** In the voter_reg_page_1 dataframe, we see that the column names are numbers (x2, x3, etc.) The removal information is taken out of the new_registrations dataset. 
...
**Task** Run the following code and describe the result.
**Answer** The removal information that was on the right side of the voter_reg_page_1 dataframe has been cleaned an put in its own data frame. The column names are each party along with a total, month, and year column. 
...
**Answer** I think that Provisional Ballots category that shows that Democrats have more than 61% of the total registering to vote using them is interesting. Also the fact that Democrats are leading by a lot in the volunteer category as well also caught my category. Even the unaffiliated voters category has more than Republicans in the volunteer category. 
...
**Task** Run the following code and describe what you think are the most interesting percentages in the dataframe.
**Answer**The transferring out of a county as a reason to remove someone's registration catches my eye. Democrats lead that category with 60% of the removal. I'm curious as to why so many of them are transferring out while only 15% of the removals in that category are from the Republican Party. I feel like the moved out of state category and the county transfer out category go together. So it kinda makes sense that Democrats would be leading in that one was well (54%)
...
**Answer** The combined dataframes has 16 rows and the June data frame has 17 rows. 
...
**Task** Run the following code. How many rows do you have compared to either the October or June dataframes?
**Answer** I see 17 rows in the combined dataframe and only 17 in the June dataframe.
...
**Answer** There's now columns for percentage difference for the Republican, Democrat, and Unaffiliated parties. "06" is used above the column names to distinguish the results from June. 
...
**Answer** The chart shows that the Eric Reports are highest percentage of the total registration for democrats at more than 60 percent while NVRA by mail is the smallest. 
...
**Answer** 
Now the charts are split in two by month: June and October. The Eric Report is leading in the October chart by percentage while Provisional Ballot is leading in the June chart. NVRA by mail is last in both charts. Absentee Ballot and motor vehicle administration are at the bottom for both charts. The other values in the chart move around between the two months. 
