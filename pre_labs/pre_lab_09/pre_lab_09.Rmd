...
**Answer**The dataset has 356 rows and 10 columns.The column names are Precinct ID, Precinct Name, Congress, Legis, Council, School, Polling ID, Shape_Length, Shape_Area,Shape.

This data set has a column called multipolygons (shapes)
...
**Answer**The data set shows the categories that people voted for at a particular precinct. 
...
**Answer**I see a map in gray and white scale split up by lines. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**We installed the tigris packages. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**We get a glimpse of the county data that will help create a geographic outline for Maryland counties. Data includes both the fipps code and county code. 
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**Code filters out results for just Maryland. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**code creates a map with defined Maryland counties but removed the gray background.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**I looks like the map was rotated and zoomed out. Bottom layer has the Maryland counties, top one has the precincts. 
...
**Answer** Reads in data that shows how people voted in each county. New dataframe called pg_turnout and filters it by county. Uses summarize to add up the number of people who voted at polls, early voting, absentee, and provisional.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**Left joins pg precincts dataset with pg_turnout data. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**The map is now color coded. The difference is that there are now white blank space where there could not be eligible voters there. This map also makes it easier to see the diversity of ways in which people votes.The dark blue spaces are closer to zero but not exactly. White spaces are actually zero.
...
**Answer*Reads in libraries for tidyverse,sf,janitor,and tidycensus.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We see the voters separated by parties in county. They are put in order of descending total. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Reads in Maryland county population data from the census. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**We see the voters by county dataset, which is seperated by county. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**Code cleans data set by changing the name column to county and removing Maryland and county from the name. Selects columns County, Population, Geometry. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
New dataframe called md_voters_per_10k by joining md_county_population and voters_by_county datasets. 
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**Spelling changes St. Mary's to Saint Mary's and Baltimore to Baltimore County in the county column.  
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**left joins md_county_population with voters_by_county. Creates new column called voters_per_10k, arranges voters_per_10k in descending order. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**Creates map with md_voters_per_10K dataset.Map key named voters_per_10k.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**Color scheme changed to warmer colors
...
**Task** Run the following code. Describe the output in the space below.
**Answer** 
Creates new column for unaffiliated population per 10,000.
```{r}
...
**Answer** 
Creates map for unaffiliated voters per 10,000. Not sure what I'm supposed to be looking for in terms of regional patters. 
```{r}
ggplot() +
