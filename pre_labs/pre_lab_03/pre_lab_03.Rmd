...
**Answer**There are 1,640 problems (as that's the number of rows.) R is saying that it is expecting a different value than what's actually popping up in these rows. (R Expects 1/0/T/F/True/False and is actually getting number values instead based on the limited information given. R just didn't calculate the value right.)
...
**Answer**There are not any parsing errors.
...
**Answer**A number data type is in the district columnm event though a value of NA is showing up. A character data type is in the precinct field. A number data type is also in the mail field. 
```{r}
...
**Answer**:402345 are in N/A. It could mean that the data is missing or someone didn't go through the process of reporting it. 
...
**Answer**13 registrations. I think the first 20 results suggets that 1900 was used as a default date. There wouldn't just be one registration for those dates this was actually accurate. 
```{r}
yadkin_voters %>% 
...
**Answer**I think looking into the people removed after 2 federal general elections in active status could be interesting. Maybe trying to find out why they were inactive. 
...
**Answer**Character is the data type for amount.
```{r}
glimpse(conowingo)
```
...
**Answer**The column headers are inconsistently styled.The amount column is stored as a character, not a number.The zip field mixes five digit ZIP codes and nine digit ZIP codes. 
...
**Answer**
It puts an X before the column name to make it easier so that it doesn't start with a number. 
...
**Answer** It says error in sumarise, problem while computing total_amount = sum(amount). Is it saying that it can't be calculated mathematically because there is a character value? 
...
**Answer**
The zip code is 21918 throughout. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
I don't see the word arnold in the results below. I don't think I'm overlooking it. 
