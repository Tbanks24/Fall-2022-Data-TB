...
```{r}
UtahSenate <- "https://electionresults.utah.gov/elections/countyCount/451556070"
...
Uresults <- Uresults[[1]] %>%  
 rename(County = 1, AKorb = 2, EMucmullin = 3, JHasen = 4, LHamblin = 5, MSeguin = 6, MLee = 7, TWilliams = 8) %>%
 slice(-2)
```
...
2018UtahResults.xlsx - downloaded

```{r}
#download.file("https://elections.utah.gov/Media/Default/2018%20Election/2018%20General%20Election%20Canvass.xlsx", "2018%20General%20Election%20Canvass.xlsx")

#USenate2018 <- read_xlsx("2018%20General%20Election%20Canvass.xlsx",
          #               sheet = )
```
...
```{r}
Utah18Senate <- read_csv("Lab13/R2018Utah2 - Sheet1.csv")
```
...
```{r}
JoinUtah <- Uresults %>% left_join(Utah18Senate, by= ("County"))%>% 
  slice(-1)
JoinUtah
```
...
```{r}
JoinUtah <- JoinUtah %>% 
  mutate(EMucmullin = as.double(EMucmullin)) %>% 
  mutate(EvanJennydiff = (EMucmullin - JWilson))
```
...
```{r}
JoinUtah <- JoinUtah %>% 
  mutate(MLee= as.double(MLee)) %>% 
  mutate(LeeRomneydiff = (MLee - MRomney))
```
...
**A3** One question you could ask is which Month this year did the most press releases get published. This could be done by filtering the different dates.You could also just pick a particular month and see how many where published then.  Based on the headlines, you could maybe determine which topics, people, or offices were discussed the most in these releases. That would mean finding common terms and then filtering it so that you only see titles with those terms. For example, you could use the term "Trump" to help find out which releases contained information on him. Although its not perfect, since titles don't always give the full story of what's in the article. 

bind_colus

```{r}
SenReleases <- "https://www.cardin.senate.gov/?post_type=press-releases" %>% 
read_html()
SenReleases
```
```{r}
SenDates<- SenReleases %>% html_elements('h5') %>% html_text() %>% as_tibble()
```


```{r}
SenTitles <- SenReleases %>% html_elements('h3 a') %>% html_text() %>% as_tibble()
```

```{r}
SenUrls <- SenReleases %>% html_elements('h3 a') %>% html_attr("href") %>% as_tibble()
```

```{r}
SenComb <- bind_cols(SenDates,SenTitles,SenUrls ) %>% 
  rename(Date = 1, Titles = 2, Url = 3) %>% 
